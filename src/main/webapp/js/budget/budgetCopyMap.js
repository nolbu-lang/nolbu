$(document).ready(function() {
    ...
    var colModel = [
                        ...
                        {name : 'adjAmt', index : 'adjAmt', width : 100, sortable : false, align : 'right', formatter : 'integer', formatoptions : {thousandsSeparator : ","}},
    ...
    var GRID_VSCROLL_RESERVE = 30; // 세로 스크롤바 폭만큼 우측 여유를 미리 확보(조정액 등 우측정렬 컬럼이 스크롤바에 가려지는 것 방지)
    ...
    var resizeListGrid = function(gridSel, paneId){
        ...
        $(gridSel, tabObj).setGridWidth($("#" + paneId, tabObj).width() - GRID_VSCROLL_RESERVE);
    ...
    var mapGridResize = function(){
        ...
        if(isEmpty(southW) == false && southW > 0){
            $("#BUDGET_MAP_GRD", tabObj).setGridWidth(southW - GRID_VSCROLL_RESERVE);
        }
    ...
    var subMainBodyResize = function(){
        ...
        resizeListGrid("#BUDGET_MAP_SRC_GRD", "subMainWest");
        resizeListGrid("#BUDGET_COPYTGT_GRD", "subMainCenter");
    };
    ...
    // 양쪽 그리드 모두 행이 선택된 경우에만 mapAddBtn 활성화
    var updateMapAddBtnState = function() {
        $("#mapAddBtn", tabObj).btnChangeState(!isEmpty(selectedSrcRowId) && !isEmpty(selectedTgtRowId));
    };
    ...
    var budgetCopySrcGrid = $.csGrid({
        ...
        rowNum : 100000,
        defaultRows : 0,
        shrinkToFit : true, // width는 비율로 사용, 그리드 폭에 맞춰 채움
        onSelectRow : function(rowId){ selectedSrcRowId = rowId; updateMapAddBtnState(); }
    });
    ...
    var budgetCopyGrid = $.csGrid({
        ...
        defaultRows : 0,
        shrinkToFit : true, // width는 비율로 사용, 그리드 폭에 맞춰 채움
        onSelectRow : function(rowId){ selectedTgtRowId = rowId; updateMapAddBtnState(); }
    });
    ...
    var doSearchSrcCallBack = function(data){
        ...
        selectedSrcRowId = "";
        updateMapAddBtnState();
        $("#BUDGET_MAP_SRC_GRD", tabObj).clearGridData();
    ...
    var doSearchCallBack = function(data){
        ...
        selectedTgtRowId = "";
        updateMapAddBtnState();
        $("#BUDGET_COPYTGT_GRD", tabObj).clearGridData();
    ...
    $("#mapDelBtn", tabObj).click(function() {
        ...
        if($("#BUDGET_MAP_GRD", tabObj)[0].rows.length === 0){
            $("#batchApplyBtn", tabObj).btnChangeState(false);
        }
    });
