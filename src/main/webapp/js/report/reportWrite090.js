$(document).ready(function() {
    var tabId = _reportWrite090TabId;
    var tabObj = $("#"+tabId);
    ...
    reportWirte090DialogDgrcompoModifyCallBackFunction = function(param){
        var rowId = param.dgrcompoId;
        if(isEmpty(rowId) == true){
            return;
        }
        
        reportWrite090Grid.jqGrid('setRowData', rowId, param);
    ...
    reportWirte090OpenDialogDgrcompoModify = function(rowId){  
        var rowData = reportWrite090Grid.getRowData(rowId);
        
        $("#dialogDgrcompoModifyCallBackFunction", $("#dialogDgrcompoModifyDiv")).val("reportWirte090DialogDgrcompoModifyCallBackFunction");
    ...
    var editFormatter = function(cellValue, options, rowObject){
        ...
        var rVal = '<a href="javascript:reportWirte090OpenDialogDgrcompoModify(\''+options.rowId+'\');"><span class="ui-icon ui-icon-pencil"></span></a>';
    ...
    var dgrcompoNmFormatter = function(cellValue, options, rowObject){
        ...
        var rVal = '<span id="dgrcompoNmView_'+rowObject.dgrcompoId+'" >' + cellValue + '</span><br>'
                 + '<textarea id="demandCont_'+rowObject.dgrcompoId+'" style="width:145px;ime-mode:active;" cols="22" rows="12">'+demandCont+'</textarea>';
  ...
  //보고항목 그리드fomatter
    ...
    var colModel = [ {name : 'edit', index : 'edit', width : 20, sortable : false, fixed : true, align : 'center', cellattr: myCellattr,
                        ...
                        {name : 'indiAttr', index : 'indiAttr', width : 100, sortable : false, hidden : false, fixed : true, align : 'left', cellattr: myCellattr,
                        	formatter:indiAttrFormatter
                        },
                        {name : 'advncProc', index : 'advncProc', width : 100, sortable : false, hidden : false, fixed : true, align : 'left', cellattr: myCellattr,
...
/*    var colNames = ['', '구분(실국-부서-세부사업)', '산출근거', '요구액', '산출근거', '조정액', '산출근거', '예산액', '산출근거', '예산액', '증감액', '비고', '재원정보', '조건검색어',
                    'dgrcompoId', 'upDgrcompoId', 'fisYear', 'bgtDgr', 'reportCd', 'reportDetlCd', 'dgrLevel', 'teBgtCompoId', 'teBgtCompoSeq', 'compoLevel', 'demandCont', 'examCont', 'reflectFg', 'srchVal'
                    ];
...
*/    
    ...
    var getGridHeight = function (){
    	...
    	$("#REPORT_WRITE090_GRD", tabObj).closest(".ui-jqgrid-bdiv").css("max-height", height + 10);
        return height;
    };
    
    var mainBodyResize = function(){
        if(isEmpty($("#REPORT_WRITE090_GRD", $("#"+tabId))) == false){
            $("#REPORT_WRITE090_GRD", $("#"+tabId)).setGridHeight(getGridHeight());
            $("#REPORT_WRITE090_GRD", $("#"+tabId)).setGridWidth($("#mainCenter", tabObj).width());
        }
    };
    ...
    var reportWrite090Grid = $("#REPORT_WRITE090_GRD", tabObj);
    ...
    var doSearchCallBack = function(data){
        ...
        $("#REPORT_WRITE090_GRD", tabObj).jqGrid('GridUnload');
        reportWrite090Grid = $("#REPORT_WRITE090_GRD", tabObj);
        reportWrite090Grid.csTreeGrid({
            datastr : data,
            ...
            loadComplete: function() {
                $(".amtInput090.ui-state-enabled", tabObj).autoNumeric({aPad: false, vMax:'99999999999999999'});
                $(".amtInput090.ui-state-enabled", tabObj).click(function () {
                    $(this).select();
                });
        ...
        $("#REPORT_WRITE090_GRD", tabObj).closest(".ui-jqgrid-bdiv").css("max-height", getGridHeight() + 10);

        reportWrite090Grid.jqGrid('setGroupHeaders', {
            useColSpanStyle : true,
            groupHeaders : groupHeaders
        });
        
        $("#REPORT_WRITE090_GRD", tabObj).closest(".ui-jqgrid-bdiv").scrollTop(gridScrollPosition);
    ...
    var doSearch = function(){
        ...
        gridScrollPosition = $("#REPORT_WRITE090_GRD", tabObj).closest(".ui-jqgrid-bdiv").scrollTop();
        
        $.csAjaxCall({
            url : "/report/ajaxReportWrite090Report090List.do",
            data: getSearchParam(),
            async : true,
  ...
  //보고항목 체크된 코드 가져오기
    ...
    var doSave = function(params){
        ...
        var saveDatas = getSaveDatas(reportWrite090Grid, $("#REPORT_WRITE090_GRD", tabObj)[0].rows);
        ...
        $.csAjaxCall({
            url : "/report/ajaxReportWrite090SaveReport090.do",
    ...
    var openDialogBgtDeptSelt = function(seltFg){
        ...
        $("#dialogDgrDeptSeltCallBackFunction", $("#dialogDgrDeptSeltDiv")).val("reportWrite090DialogDgrDeptSeltCallBack");
        ...
        $("#dialogDgrDeptSeltReportCd", $("#dialogDgrDeptSeltDiv")).val("090");
        $("#dialogDgrDeptSeltUserDeptYn", $("#dialogDgrDeptSeltDiv")).val("");
        
        $("#dialogDgrDeptSeltDiv").dialog('open');
    };
    
    reportWrite090DialogDgrDeptSeltCallBack = function(param){
    ...
    var comboParam = [
                      ...
                      {id : "officeCd", subQueryId : "OfficeCd", reportCd: "090"},
