$(document).ready(function() {
    var tabId = _reportWrite050TabId;
    var tabObj = $("#"+tabId);
    ...
    reportWirte050DialogDgrcompoModifyCallBackFunction = function(param){
        var rowId = param.dgrcompoId;
        if(isEmpty(rowId) == true){
            return;
        }
        
        reportWrite050Grid.jqGrid('setRowData', rowId, param);
    ...
    reportWirte050OpenDialogDgrcompoModify = function(rowId){  
        var rowData = reportWrite050Grid.getRowData(rowId);
        
        $("#dialogDgrcompoModifyCallBackFunction", $("#dialogDgrcompoModifyDiv")).val("reportWirte050DialogDgrcompoModifyCallBackFunction");
    ...
    var editFormatter = function(cellValue, options, rowObject){
        ...
        var rVal = '<a href="javascript:reportWirte050OpenDialogDgrcompoModify(\''+options.rowId+'\');"><span class="ui-icon ui-icon-pencil"></span></a>';
    ...
    var dgrcompoNmFormatter = function(cellValue, options, rowObject){
        ...
        var rVal = '<span id="dgrcompoNmView_'+rowObject.dgrcompoId+'" >' + cellValue + '</span><br>'
                 + '<textarea id="demandCont_'+rowObject.dgrcompoId+'" style="width:230px;ime-mode:active;height:20px;" cols="22" rows="12">'+demandCont+'</textarea>';
    ...
    var examContFormatter = function(cellValue, options, rowObject){
        ...
        var rVal = '<div>'
                 + '<select id="reflectFg_'+rowObject.dgrcompoId+'" title="반영구분" style="width:262px;">'
                 + reflectFgCreateCombo('RP003', rowObject.reflectFg)
                 + '</select>'+'<br>'
                 + '<textarea id="examCont_'+rowObject.dgrcompoId+'" style="width:260px;ime-mode:active;height:20px;" cols="22" rows="12">'+cellValue+'</textarea>'
                 + '</div>';
  ...
  //분류항목 그리드fomatter
    ...
    var colNames = ['', '구분(회계구분-실국-부서-세부사업)', '요구액', '조정액(A)', '본예산', '전년도예산액(B)', '증감액(A-B)', '비고', '재원정보', '조건검색어',
    ...
    var colModel = [ {name : 'edit', index : 'edit', width : 20, sortable : false, fixed : true, align : 'center', cellattr: myCellattr,
                        ...
                        {name : 'dgrcompoNm', index : 'dgrcompoNm', width : 350, sortable : false, fixed : true, align : 'left', cellattr: myCellattr,
...
*/    
    ...
    var getGridHeight = function (){
    	...
    	$("#REPORT_WRITE050_GRD", tabObj).closest(".ui-jqgrid-bdiv").css("max-height", height - 10);
        return height;
    };
    
    var mainBodyResize = function(){
        if(isEmpty($("#REPORT_WRITE050_GRD", $("#"+tabId))) == false){
            $("#REPORT_WRITE050_GRD", $("#"+tabId)).setGridHeight(getGridHeight());
            $("#REPORT_WRITE050_GRD", $("#"+tabId)).setGridWidth($("#mainCenter", tabObj).width());
        }
    };
    ...
    var reportWrite050Grid = $("#REPORT_WRITE050_GRD", tabObj);
    ...
    var doSearchCallBack = function(data){
        ...
        if(data.data.bgtDgr == "1"){
            ...
            groupHeaders = [{startColumnName : 'demandBgtAmt',numberOfColumns : 2, titleText : '당해연도'}];
        }else{
            colModel[4].hidden = false;
            colModel[5].hidden = true;
            colModel[6].hidden = true;
            groupHeaders = [{startColumnName : 'demandBgtAmt',numberOfColumns : 2, titleText : '추경'}];
        }

        $("#REPORT_WRITE050_GRD", tabObj).jqGrid('GridUnload');
        reportWrite050Grid = $("#REPORT_WRITE050_GRD", tabObj);
        reportWrite050Grid.csTreeGrid({
            datastr : data,
        ...
        $("#REPORT_WRITE050_GRD", tabObj).closest(".ui-jqgrid-bdiv").css("max-height", getGridHeight() - 10);

        reportWrite050Grid.jqGrid('setGroupHeaders', {
            useColSpanStyle : true,
            groupHeaders : groupHeaders
        });
        
        $("#REPORT_WRITE050_GRD", tabObj).closest(".ui-jqgrid-bdiv").scrollTop(gridScrollPosition);
    ...
    var doSearch = function(){
        ...
        gridScrollPosition = $("#REPORT_WRITE050_GRD", tabObj).closest(".ui-jqgrid-bdiv").scrollTop();
        
        $.csAjaxCall({
            url : "/report/ajaxReportWrite050Report050List.do",
            data: getSearchParam(),
            async : true,
  ...
  //보고항목 체크된 코드 가져오기
    ...
    var doSave = function(params){
        ...
        var saveDatas = getSaveDatas(reportWrite050Grid, $("#REPORT_WRITE050_GRD", tabObj)[0].rows);
        ...
        $.csAjaxCall({
            url : "/report/ajaxReportWrite050SaveReport050.do",
    ...
    $("#saveFileBtn", tabObj).click(function() {
        ...
        /*$.bcjisExcelAjaxCall({
            url : "/report/ajaxReportWrite050SaveFile.do"
          , data: param
        });*/
    });
    ...
    $("#saveSheetBtn", tabObj).click(function() {
        ...
        /*$.bcjisExcelAjaxCall({
            url : "/report/ajaxReportWrite050SaveSheet.do"
          , data: param
        });*/
    });
    ...
    var openDialogBgtDeptSelt = function(seltFg){
        ...
        $("#dialogDgrDeptSeltCallBackFunction", $("#dialogDgrDeptSeltDiv")).val("reportWrite050DialogDgrDeptSeltCallBack");
        ...
        $("#dialogDgrDeptSeltReportCd", $("#dialogDgrDeptSeltDiv")).val("050");
        $("#dialogDgrDeptSeltUserDeptYn", $("#dialogDgrDeptSeltDiv")).val("");
        
        $("#dialogDgrDeptSeltDiv").dialog('open');
    };
    
    reportWrite050DialogDgrDeptSeltCallBack = function(param){
    ...
    var comboParam = [
					...
					{id : "advncProc", codeId : "RP015"},
                      ...
                      {id : "officeCd", subQueryId : "OfficeCd", reportCd: "050"},
    ...
    var chkDiv = function(){
    	checkEvent($('#REPORT_WRITE050_DIV').find('textarea'));
    	checkEvent($('#REPORT_WRITE050_DIV').find('input'));
    	checkEvent($('#REPORT_WRITE050_DIV').find('select'));
    }
