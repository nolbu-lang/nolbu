$(document).ready(function() {
    ...
    var guGoonExpFormatter = function(cellValue, options, rowObject){
        ...
        var rVal = '<input id="guGoonExp_'+rowObject.dgrcompoId+'" value="'+addCommaMinusStr(cellValue)+'" '+classStr+' '+readOnlyStr+' />';
        
        return rVal;
    };
    
    var preBgtAmtExpFormatter = function(cellValue, options, rowObject){
    	if(isEmpty(cellValue) == true){
    		cellValue = "";
    	}
    	
    	var readOnlyStr = "";
    	var classStr = 'class="amtInput030 ui-state-enabled"';
    	/*if(rowObject.dbizCd == "0000000000000000"){
    		readOnlyStr = "readonly";
    		classStr = 'class="amtInput030 ui-state-disabled"';
    	}else{
    	...
    	var rVal = ''; 
    	if(rowObject.dbizCd != "0000000000000000"){
    	rVal = '<input id="preBgtAmtExp_'+rowObject.dgrcompoId+'" value="'+addCommaMinusStr(cellValue)+'" '+classStr+' '+readOnlyStr+' />';
    	}
    ...
    var colNames = ['구분(부서-사업)', '통계목', '계', '국비', '시비', '계', '국비', '시비', '계', '국비', '시비', '구·군비(※별도)', '국가사업명', '부처명', '국비', '시비', '구비', '전년도 예산', '비고', '조건검색어',
                    '국고보조',
                    ...
                    'guGoonExp', 'preBgtAmtExp', 'natnBizNm', 'mnstryNm', 'natnBudnRate', 'siBudnRate', 'guBudnRate',
                    'indiAttr','advncProc'
                   ];
    ...
    var colModel = [ {name : 'dgrcompoNm', index : 'dgrcompoNm', width : 260, sortable : false, fixed : true, align : 'left', cellattr: myCellattr},
                        ...
                        {name : 'preBgtAmtExpView', index : 'preBgtAmtExpView', width : 90, sortable : false, fixed : true, align : 'center', cellattr: myCellattr,
                            formatter:preBgtAmtExpFormatter
                        },
                        ...
                        {name : 'preBgtAmtExp', index : 'preBgtAmtExp', width : 0, sortable : false, hidden : true},
...
*/
    var getGridHeight = function (){
    	...
    	$("#REPORT_WRITE030_GRD", tabObj).closest(".ui-jqgrid-bdiv").css("max-height", height - 10);
        return height;
    };
    
    var mainBodyResize = function(){
        if(isEmpty($("#REPORT_WRITE030_GRD", $("#"+tabId))) == false){
            $("#REPORT_WRITE030_GRD", $("#"+tabId)).setGridHeight(getGridHeight());
            $("#REPORT_WRITE030_GRD", $("#"+tabId)).setGridWidth($("#mainCenter", tabObj).width());
        }
    };
    ...
    var reportWrite030Grid = $("#REPORT_WRITE030_GRD", tabObj);
    ...
    var doSearchCallBack = function(data){
        ...
        $("#REPORT_WRITE030_GRD", tabObj).jqGrid('GridUnload');
        reportWrite030Grid = $("#REPORT_WRITE030_GRD", tabObj);
        reportWrite030Grid.csTreeGrid({
            ...
            loadComplete: function() {
                ...
                $(".amtInput030.ui-state-enabled", tabObj).autoNumeric('init', {aPad: false, vMin:'-99999999999999999', vMax:'99999999999999999'});
        ...
        //$("#REPORT_WRITE030_GRD", tabObj).closest(".ui-jqgrid-bdiv").css("max-height", gridHeight - $("#REPORT_WRITE030_GRD", tabObj).closest(".ui-jqgrid-hdiv").height());
        $("#REPORT_WRITE030_GRD", tabObj).closest(".ui-jqgrid-bdiv").css("max-height", getGridHeight() - 10);
        ...
        $("#REPORT_WRITE030_GRD", tabObj).closest(".ui-jqgrid-bdiv").scrollTop(gridScrollPosition);
    ...
    var doSearch = function(){
        ...
        $.csAjaxCall({
            url : "/report/ajaxReportWrite030Report030List.do",
            data: getSearchParam(),
            async : true,
        ...
        for(var i = 0; i < gridRows.length; i++) {
            ...
            if(isEmpty(rowData.dgrcompoId) == false && rowData.dbizCd != "0000000000000000"){
                ...
                if(rowData.examCont != examCont
                        ...
                        || rowData.preBgtAmtExp != preBgtAmtExp){
                    ...
                    saveData["preBgtAmtExp"] = preBgtAmtExp;
  ...
  //보고항목 체크된 코드 가져오기
    ...
    var doSave = function(params){
        ...
        var saveDatas = getSaveDatas(reportWrite030Grid, $("#REPORT_WRITE030_GRD", tabObj)[0].rows);
        ...
        $.csAjaxCall({
            url : "/report/ajaxReportWrite030SaveReport030.do",
    ...
    $("#saveFileBtn", tabObj).click(function() {
        ...
        /*$.bcjisExcelAjaxCall({
            url : "/report/ajaxReportWrite030SaveFile.do"
          , data: param
        });*/
    });
    ...
    $("#saveSheetBtn", tabObj).click(function() {
        ...
        /*$.bcjisExcelAjaxCall({
            url : "/report/ajaxReportWrite030SaveSheet.do"
          , data: param
        });*/
    });
    ...
    var condReportDetlCdCreateCombo = function(groupId, selectedValue){
        ...
        if($("#condReportDetlCd option:eq(0)", tabObj)){
        	var allTxt = $("#condReportDetlCd option:eq(0)", tabObj).text();
            if(allTxt == '전체'){
            	$("#condReportDetlCd option:eq(0)", tabObj).text('전체(매칭펀드 제외)');
            }
        }
    ...
    var chkDiv = function(){
    	checkEvent($('#REPORT_WRITE030_DIV').find('textarea'));
    	checkEvent($('#REPORT_WRITE030_DIV').find('input'));
    }
