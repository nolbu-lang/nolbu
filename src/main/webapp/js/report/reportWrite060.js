$(document).ready(function() {
    var tabId = _reportWrite060TabId;
    var tabObj = $("#"+tabId);
    ...
    reportWirte060DialogDgrcompoModifyCallBackFunction = function(param){
        var rowId = param.dgrcompoId;
        if(isEmpty(rowId) == true){
            return;
        }
        
        reportWrite060Grid.jqGrid('setRowData', rowId, param);
    ...
    reportWirte060OpenDialogDgrcompoModify = function(rowId){  
        var rowData = reportWrite060Grid.getRowData(rowId);
        
        $("#dialogDgrcompoModifyCallBackFunction", $("#dialogDgrcompoModifyDiv")).val("reportWirte060DialogDgrcompoModifyCallBackFunction");
    ...
    var editFormatter = function(cellValue, options, rowObject){
        ...
        var rVal = '<a href="javascript:reportWirte060OpenDialogDgrcompoModify(\''+options.rowId+'\');"><span class="ui-icon ui-icon-pencil"></span></a>';
    ...
    var dgrcompoNmFormatter = function(cellValue, options, rowObject){
        ...
        var rVal = '<span id="dgrcompoNmView_'+rowObject.dgrcompoId+'" >' + cellValue + '</span><br>'
                 + '<textarea id="demandCont_'+rowObject.dgrcompoId+'" style="width:200px;ime-mode:active;" cols="22" rows="12">'+demandCont+'</textarea>';
...
/*    var colNames = ['', '구분(실국-부서-세부사업)', '산출근거식', '요구액', '산출근거식', '조정액', '산출근거', '예산액', '산출근거', '예산액', '증감액', '검토내용', '재원정보', '조건검색어',
                    'dgrcompoId', 'upDgrcompoId', 'fisYear', 'bgtDgr', 'reportCd', 'reportDetlCd', 'dgrLevel', 'teBgtCompoId', 'teBgtCompoSeq', 'compoLevel', 'demandCont', 'examCont', 'reflectFg', 'srchVal'
                    ];
    ...
    {name : 'bgtAmt', index : 'bgtAmt', width : 80, sortable : false, fixed : true, align : 'right', formatter : 'integer', formatoptions : {thousandsSeparator : ","}, cellattr: myCellattr},
    {name : 'preFormular', index : 'preFormular', width : 80, sortable : false, fixed : true, align : 'left', cellattr: myCellattr},
    {name : 'preAmt', index : 'preAmt', width : 80, sortable : false, fixed : true, align : 'right', formatter : 'integer', formatoptions : {thousandsSeparator : ","}, cellattr: myCellattr},
...
*/    
    ...
    var getGridHeight = function (){
    	...
    	$("#REPORT_WRITE060_GRD", tabObj).closest(".ui-jqgrid-bdiv").css("max-height", height - 10);
        return height;
    };
    
    var mainBodyResize = function(){
        if(isEmpty($("#REPORT_WRITE060_GRD", $("#"+tabId))) == false){
            $("#REPORT_WRITE060_GRD", $("#"+tabId)).setGridHeight(getGridHeight());
            $("#REPORT_WRITE060_GRD", $("#"+tabId)).setGridWidth($("#mainCenter", tabObj).width());
        }
    };
    ...
    var reportWrite060Grid = $("#REPORT_WRITE060_GRD", tabObj);
    ...
    var doSearchCallBack = function(data){
        ...
        if(data.data.bgtDgr == "1"){
            ...
            groupHeaders = [
                            {startColumnName : 'demandCompFormular',numberOfColumns : 2, titleText : '당해연도 요구안'},
                            {startColumnName : 'compFormular', numberOfColumns : 2, titleText : '조정안'} ,
                            {startColumnName : 'preCompFormular', numberOfColumns : 2, titleText : '전년도 예산(추경포함)'} ,
            ...
            groupHeaders = [
                            {startColumnName : 'demandCompFormular',numberOfColumns : 2, titleText : '추경 요구안'},
                            {startColumnName : 'compFormular', numberOfColumns : 2, titleText : '조정안'} ,
                            {startColumnName : 'preCompFormular', numberOfColumns : 2, titleText : '전년도 예산(추경포함)'} ,
        ...
        $("#REPORT_WRITE060_GRD", tabObj).jqGrid('GridUnload');
        reportWrite060Grid = $("#REPORT_WRITE060_GRD", tabObj);
        reportWrite060Grid.csTreeGrid({
            datastr : data,
        ...
        $("#REPORT_WRITE060_GRD", tabObj).closest(".ui-jqgrid-bdiv").css("max-height", getGridHeight() - 10);

        reportWrite060Grid.jqGrid('setGroupHeaders', {
            useColSpanStyle : true,
            groupHeaders : groupHeaders
        });
        
        $("#REPORT_WRITE060_GRD", tabObj).closest(".ui-jqgrid-bdiv").scrollTop(gridScrollPosition);
    ...
    var doSearch = function(){
        ...
        gridScrollPosition = $("#REPORT_WRITE060_GRD", tabObj).closest(".ui-jqgrid-bdiv").scrollTop();
        
        $.csAjaxCall({
            url : "/report/ajaxReportWrite060Report060List.do",
            data: getSearchParam(),
            async : true,
    ...
    var doSave = function(params){
        ...
        var saveDatas = getSaveDatas(reportWrite060Grid, $("#REPORT_WRITE060_GRD", tabObj)[0].rows);
        ...
        $.csAjaxCall({
            url : "/report/ajaxReportWrite060SaveReport060.do",
    ...
    var openDialogBgtDeptSelt = function(seltFg){
        ...
        $("#dialogDgrDeptSeltCallBackFunction", $("#dialogDgrDeptSeltDiv")).val("reportWrite060DialogDgrDeptSeltCallBack");
        ...
        $("#dialogDgrDeptSeltReportCd", $("#dialogDgrDeptSeltDiv")).val("060");
        $("#dialogDgrDeptSeltUserDeptYn", $("#dialogDgrDeptSeltDiv")).val("");
        
        $("#dialogDgrDeptSeltDiv").dialog('open');
    };
    
    reportWrite060DialogDgrDeptSeltCallBack = function(param){
    ...
    var comboParam = [
                      ...
                      {id : "officeCd", subQueryId : "OfficeCd", reportCd: "060"},
