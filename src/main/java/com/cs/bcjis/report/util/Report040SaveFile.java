...
@Component("report040SaveFile")
public class Report040SaveFile {
    ...
    public void data(Map model, XSSFWorkbook wb, List<Object> categories) throws Exception {
        ...
        while (!categories.isEmpty()) {
            ...
            if (dgrLevel == 0) {
                ...
                if ("043".equals(reportDetlCd) == true) {
                    rowNum = writeHeader(model, sheet, rowNum, styles, reportInfo, ReportSaveUtil.getStringValue(reportInfo.get("reportNm")), 14);
                } else if ("045".equals(reportDetlCd) == true) {
                    rowNum = writeHeader(model, sheet, rowNum, styles, reportInfo, ReportSaveUtil.getStringValue(reportInfo.get("reportNm")), 10);
                } else {
                    rowNum = writeHeader(model, sheet, rowNum, styles, reportInfo, ReportSaveUtil.getStringValue(reportInfo.get("reportNm")), 16);
                }

            }

            if ("043".equals(reportDetlCd) == true) {
                rowNum = writeData043(sheet, rowNum, category, styles, bgtCompoFlag, reportFormulaUtil);
            } else if ("045".equals(reportDetlCd) == true) {
                rowNum = writeData045(sheet, rowNum, category, styles, bgtCompoFlag, reportFormulaUtil);
            } else {
    ...
    public int writeData(XSSFSheet sheet, int rowNum, JSONObject category, Map<String, CellStyle> styles, boolean bgtCompoFlag, ReportFormulaUtil reportFormulaUtil) throws Exception {
        ...
        if (formulaFlag == false) {
            cell.setCellValue(ReportSaveUtil.getAmtValue(category.get("staffCnt")));
        }

        cell = row.createCell(4);
        cell.setCellStyle(styles.get(preStyleNm + "Col4"));
        if (formulaFlag == false) {
            cell.setCellValue(ReportSaveUtil.getAmtValue(category.get("staffCurrCnt")));
        }
        ...
        if (formulaFlag == false) {
            cell.setCellValue(ReportSaveUtil.getAmtValue(category.get("driverCnt")));
        }

        cell = row.createCell(7);
        cell.setCellStyle(styles.get(preStyleNm + "Col7"));
        if (formulaFlag == false) {
            cell.setCellValue(ReportSaveUtil.getAmtValue(category.get("etcCnt")));
        }
        ...
        System.out.println(" ROW : " + rowNum + "    : " + ReportSaveUtil.getStringValue(category.get("demandCompFormular")));
    ...
    public int writeData043(XSSFSheet sheet, int rowNum, JSONObject category, Map<String, CellStyle> styles, boolean bgtCompoFlag, ReportFormulaUtil reportFormulaUtil) throws Exception {
        float rowHeight = 30f;
        Row row = null;
        Cell cell = null;
        ...
        cell = row.createCell(13);
        cell.setCellStyle(styles.get(preStyleNm + "Col13"));
        cell.setCellFormula("K" + (rowNum) + "-" + "M" + (rowNum));
        ...
        if (dgrLevel < 5) {
            addData043FormulaValue(reportFormulaUtil, upDgrcompoId, rowNum);
        }

        if (formulaFlag == true) {
            addData043FormulaCell(reportFormulaUtil, dgrcompoId, rowNum);
        }
    ...
    public int writeData045(XSSFSheet sheet, int rowNum, JSONObject category, Map<String, CellStyle> styles, boolean bgtCompoFlag, ReportFormulaUtil reportFormulaUtil) throws Exception {
        float rowHeight = 30f;
        Row row = null;
        Cell cell = null;
        ...
        if (dgrLevel < 5) {
            addData045FormulaValue(reportFormulaUtil, upDgrcompoId, rowNum);
        }

        if (formulaFlag == true) {
            addData045FormulaCell(reportFormulaUtil, dgrcompoId, rowNum);
        }
    ...
    public void addData043FormulaValue(ReportFormulaUtil reportFormulaUtil, String keyStr, int rowNum) {
        reportFormulaUtil.addFormulaValue(keyStr + "_D", "D" + rowNum);
        reportFormulaUtil.addFormulaValue(keyStr + "_E", "E" + rowNum);
    ...
    public void addData043FormulaCell(ReportFormulaUtil reportFormulaUtil, String keyStr, int rowNum) {
        reportFormulaUtil.addFormulaCell(keyStr + "_D", rowNum - 1, 3);
        reportFormulaUtil.addFormulaCell(keyStr + "_E", rowNum - 1, 4);
    ...
    public void addData045FormulaValue(ReportFormulaUtil reportFormulaUtil, String keyStr, int rowNum) {
        reportFormulaUtil.addFormulaValue(keyStr + "_E", "E" + rowNum);
        reportFormulaUtil.addFormulaValue(keyStr + "_G", "G" + rowNum);
        reportFormulaUtil.addFormulaValue(keyStr + "_I", "I" + rowNum);
    }

    public void addData045FormulaCell(ReportFormulaUtil reportFormulaUtil, String keyStr, int rowNum) {
        reportFormulaUtil.addFormulaCell(keyStr + "_E", rowNum - 1, 4);
        reportFormulaUtil.addFormulaCell(keyStr + "_G", rowNum - 1, 6);
        reportFormulaUtil.addFormulaCell(keyStr + "_I", rowNum - 1, 8);
    }
    ...
    public void dataList(Map model, XSSFWorkbook wb, List<Object> categories) throws Exception {
        ...
        if("041".equals(reportDetlCd)){
        	title = "기본수용비";
        }else if("042".equals(reportDetlCd)){
        	title = "급량비";
        }else if("043".equals(reportDetlCd)){
        	title = "당직수당";
        }else if("044".equals(reportDetlCd)){
        	title = "국내여비";
        }else if("045".equals(reportDetlCd)){
        	title = "월액여비";
        }else{
        	title = "기본경비";
        }
