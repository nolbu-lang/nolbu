...
@Component("report080SaveFile")
public class Report080SaveFile {
    ...
    public void data(Map model, XSSFWorkbook wb, List<Object> categories) throws Exception {
        ...
        while (!categories.isEmpty()) {
            ...
            if (reportDetlCd.equals(preReportDetlCd) == false) {
                if (sheet != null) {
                ...
                rowNum = writeHeader(model, sheet, rowNum, styles, reportInfo, ReportSaveUtil.getStringValue(reportInfo.get("reportNm")), 8);

            }

            dataCnt++;
            rowNum = writeData(sheet, rowNum, category, styles, dataCnt, bgtCompoFlag);

            preReportDetlCd = reportDetlCd;
        }
    ...
    public int writeBizlistData(XSSFSheet sheet, int rowNum, JSONObject category, Map<String, CellStyle> styles, int totDataCnt, int dataCnt, int subCnt, boolean bgtCompoFlag, ReportFormulaUtil reportFormulaUtil) {
        ...
        cell.setCellValue(ReportSaveUtil.getStringValue(category.get("importPersonNm")));
    ...
    public int writeData(XSSFSheet sheet, int rowNum, JSONObject category, Map<String, CellStyle> styles, int dataCnt, boolean bgtCompoFlag) throws Exception {
        ...
        List<Report080SaveFrscAmtsVO> frscAmts = getFrscAmts(category, bgtCompoFlag);

        String frscNm = "";
        long dmnFrscAmt = 0l;
        long preFrscAmt = 0l;
        long frscAmt = 0l;
        Report080SaveFrscAmtsVO report080SaveFrscAmtsVO = null;
        if (!frscAmts.isEmpty()) {
            report080SaveFrscAmtsVO = frscAmts.remove(0);
            frscNm = report080SaveFrscAmtsVO.getFrscNm();
            dmnFrscAmt = report080SaveFrscAmtsVO.getDmnFrscAmt();
            preFrscAmt = report080SaveFrscAmtsVO.getPreFrscAmt();
            frscAmt = report080SaveFrscAmtsVO.getFrscAmt();
        }
        ...
        cell = row.createCell(3);
        cell.setCellStyle(styles.get(preStyleNm + "Col3"));
        long preBizAmt = ReportSaveUtil.getAmtValue(category.get("preBizAmt"));
        if (preBizAmt != 0) {
            cell.setCellValue(preBizAmt);
        }
        ...
        sheet.addMergedRegion(CellRangeAddress.valueOf("$C$" + (rowNum - 1) + ":$C$" + rowNum));
    ...
    public List<Report080SaveFrscAmtsVO> getFrscAmts(JSONObject category, boolean bgtCompoFlag) {
        List<Report080SaveFrscAmtsVO> list = new ArrayList<Report080SaveFrscAmtsVO>();

        long dmnFrscAmt = ReportSaveUtil.getAmtValue(category.get("dmnFrscAmt1"));
        long preFrscAmt = 0;
        if (bgtCompoFlag == false) {
            preFrscAmt = ReportSaveUtil.getAmtValue(category.get("preDefFrscAmt1"));
        } else {
            preFrscAmt = ReportSaveUtil.getAmtValue(category.get("preFrscAmt1"));
        }

        long frscAmt = ReportSaveUtil.getAmtValue(category.get("frscAmt1"));

        if (dmnFrscAmt != 0 || preFrscAmt != 0 || frscAmt != 0) {
            list.add(new Report080SaveFrscAmtsVO("시", dmnFrscAmt, preFrscAmt, frscAmt));
        }

        dmnFrscAmt = ReportSaveUtil.getAmtValue(category.get("dmnFrscAmt2"));
        if (bgtCompoFlag == false) {
            preFrscAmt = ReportSaveUtil.getAmtValue(category.get("preDefFrscAmt2"));
        } else {
        ...
        if (dmnFrscAmt != 0 || preFrscAmt != 0 || frscAmt != 0) {
            list.add(new Report080SaveFrscAmtsVO("국", dmnFrscAmt, preFrscAmt, frscAmt));
        }

        dmnFrscAmt = ReportSaveUtil.getAmtValue(category.get("dmnFrscAmt3"));
        if (bgtCompoFlag == false) {
            preFrscAmt = ReportSaveUtil.getAmtValue(category.get("preDefFrscAmt3"));
        } else {
        ...
        if (dmnFrscAmt != 0 || preFrscAmt != 0 || frscAmt != 0) {
            list.add(new Report080SaveFrscAmtsVO("교", dmnFrscAmt, preFrscAmt, frscAmt));
        }

        dmnFrscAmt = ReportSaveUtil.getAmtValue(category.get("dmnFrscAmt4"));
        if (bgtCompoFlag == false) {
            preFrscAmt = ReportSaveUtil.getAmtValue(category.get("preDefFrscAmt4"));
        } else {
        ...
        if (dmnFrscAmt != 0 || preFrscAmt != 0 || frscAmt != 0) {
            list.add(new Report080SaveFrscAmtsVO("지", dmnFrscAmt, preFrscAmt, frscAmt));
        }

        dmnFrscAmt = ReportSaveUtil.getAmtValue(category.get("dmnFrscAmt5"));
        if (bgtCompoFlag == false) {
            preFrscAmt = ReportSaveUtil.getAmtValue(category.get("preDefFrscAmt5"));
        } else {
        ...
        if (dmnFrscAmt != 0 || preFrscAmt != 0 || frscAmt != 0) {
            list.add(new Report080SaveFrscAmtsVO("채", dmnFrscAmt, preFrscAmt, frscAmt));
        }

        dmnFrscAmt = ReportSaveUtil.getAmtValue(category.get("dmnFrscAmt6"));
        if (bgtCompoFlag == false) {
            preFrscAmt = ReportSaveUtil.getAmtValue(category.get("preDefFrscAmt6"));
        } else {
        ...
        if (dmnFrscAmt != 0 || preFrscAmt != 0 || frscAmt != 0) {
            list.add(new Report080SaveFrscAmtsVO("기", dmnFrscAmt, preFrscAmt, frscAmt));
        }
