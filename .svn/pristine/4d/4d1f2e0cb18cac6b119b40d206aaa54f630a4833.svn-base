...
@Controller
public class BudgetSheetSelectController {
    /**
     * Logger for this class
     */
    private static final Logger logger = Logger.getLogger(BudgetSheetSelectController.class);
    ...
    @RequestMapping("/budget/budgetSheetSelect.do")
    public String budgetSelect(Map<String, String> commandMap, ModelMap model, HttpServletRequest request) throws Exception {
        ...
        return "budget/budgetSheetSelect";
    }
    
    @RequestMapping("/budget/ajaxBudgetSheetSelectDgrCompoList.do")
    public ModelAndView ajaxBudgetSelectDgrCompoList(ModelMap model, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            JSONArray resultList = JSONArray.fromObject(budgetSheetSelectService.selectDgrCompoList(jsonParam));
    ...
    @RequestMapping("/budget/ajaxBudgetSheetSelectSaveSheet.do")
    public ModelAndView ajaxBudgetSelectSaveSheet(ModelMap model, HttpServletRequest request) throws Exception {
        if (logger.isDebugEnabled()) {
            logger.debug("ajaxBudgetSelectSaveSheet(ModelMap, HttpServletRequest) - start");
        }
        ...
        try {
            ...
            budgetSheetSelectService.saveSheet(jsonParam);

            jsonObject.put(BcjisCommUtil.BCJIS_RETURN_CODE, BcjisCommUtil.BCJIS_RETURN_CODE_SUCC);
            jsonObject.put(BcjisCommUtil.BCJIS_MESSAGE, bcjisMessageSource.getMessage("success.common.save"));
        } catch (Exception e) {
            logger.error("ajaxBudgetSelectSaveSheet(ModelMap, HttpServletRequest)", e);
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("ajaxBudgetSelectSaveSheet(ModelMap, HttpServletRequest) - end");
        }
        return ajaxModel;
    }
}
