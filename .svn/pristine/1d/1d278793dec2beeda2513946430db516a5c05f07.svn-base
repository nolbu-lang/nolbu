...
@Component("report060SaveFile")
public class Report060SaveFile {
    ...
    public int writeCover(Map model, XSSFWorkbook wb) throws Exception {
        ...
        wb.setPrintArea(wb.getSheetIndex(sheet), 0, 9, 0, rowNum - 1);
    ...
    public void data(Map model, XSSFWorkbook wb, List<Object> categories) throws Exception {
        ...
        while (!categories.isEmpty()) {
            ...
            if (dgrLevel == 0) {
                if (sheet != null) {
                    ReportSaveUtil.writeLastSheet(reportCommDAO, model, wb, sheet, rowNum, styles, reportInfo, 9);
                ...
                if("066".equals(reportDetlCd) == true){
                    rowNum = writeHeader066(model, sheet, rowNum, styles, reportInfo, ReportSaveUtil.getStringValue(reportInfo.get("reportNm")), 10);
                }else{
                    rowNum = writeHeader(model, sheet, rowNum, styles, reportInfo, ReportSaveUtil.getStringValue(reportInfo.get("reportNm")), 9);
                }

            }

            if("066".equals(reportDetlCd) == true){
                rowNum = writeData066(sheet, rowNum, category, styles, bgtCompoFlag, reportFormulaUtil);
            }else{
    ...
    public int writeHeader(Map model, XSSFSheet sheet, int rowNum, Map<String, CellStyle> styles, Map reportInfo, String title, int unitPos) throws Exception {
        ...
        sheet.addMergedRegion(CellRangeAddress.valueOf("$A$" + (rowNum - 1) + ":$C$" + (rowNum) + ""));
        sheet.addMergedRegion(CellRangeAddress.valueOf("$D$" + (rowNum - 1) + ":$E$" + (rowNum - 1) + ""));
        sheet.addMergedRegion(CellRangeAddress.valueOf("$F$" + (rowNum - 1) + ":$G$" + (rowNum - 1) + ""));
        sheet.addMergedRegion(CellRangeAddress.valueOf("$H$" + (rowNum - 1) + ":$I$" + (rowNum - 1) + ""));
        sheet.addMergedRegion(CellRangeAddress.valueOf("$J$" + (rowNum - 1) + ":$J$" + (rowNum) + ""));
        sheet.addMergedRegion(CellRangeAddress.valueOf("$K$" + (rowNum - 1) + ":$Q$" + (rowNum - 1) + ""));
        sheet.addMergedRegion(CellRangeAddress.valueOf("$R$" + (rowNum - 1) + ":$R$" + (rowNum) + ""));
        ...
        sheet.createFreezePane(3, rowNum + 1);
    ...
    public int writeReportCont(Map param, XSSFSheet sheet, int rowNum, Map<String, CellStyle> styles) throws Exception {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (i == 0 && "066".equals(param.get("reportDetlCd")) == true) {
    ...
    public int writeData(XSSFSheet sheet, int rowNum, JSONObject category, Map<String, CellStyle> styles, boolean bgtCompoFlag, ReportFormulaUtil reportFormulaUtil) throws Exception {
        ...
        cell.setCellFormula("L" + rowNum + "+" + "M" + rowNum + "+" + "N" + rowNum + "+" + "O" + rowNum + "+" + "P" + rowNum + "+" + "Q" + rowNum);
        ...
        cell.setCellStyle(styles.get(preStyleNm + "Col17"));
    ...
    /*066*/
    @SuppressWarnings("rawtypes")
    public int writeHeader066(Map model, XSSFSheet sheet, int rowNum, Map<String, CellStyle> styles, Map reportInfo, String title, int unitPos) throws Exception {
        Row row = null;
        Cell cell = null;
        ...
        rowNum = writeReportCont066(model, sheet, rowNum, styles);
    ...
    @SuppressWarnings("rawtypes")
    public int writeReportCont066(Map param, XSSFSheet sheet, int rowNum, Map<String, CellStyle> styles) throws Exception {
    ...
    public int writeData066(XSSFSheet sheet, int rowNum, JSONObject category, Map<String, CellStyle> styles, boolean bgtCompoFlag, ReportFormulaUtil reportFormulaUtil) throws Exception {
        float rowHeight = 27f;
        Row row = null;
        Cell cell = null;
        ...
        cell = row.createCell(11);
        cell.setCellStyle(styles.get(preStyleNm + "Col11"));
        cell.setCellFormula("M" + rowNum + "+" + "N" + rowNum + "+" + "O" + rowNum + "+" + "P" + rowNum + "+" + "Q" + rowNum + "+" + "R" + rowNum);
        ...
        cell.setCellStyle(styles.get(preStyleNm + "Col18"));
        cell.setCellValue(ReportSaveUtil.getStringValue(category.get("srchVal")));
        ...
        if (dgrLevel < 5) {
            addDataFormulaValue066(reportFormulaUtil, upDgrcompoId, rowNum);
        }

        if (formulaFlag == true) {
            addDataFormulaCell066(reportFormulaUtil, dgrcompoId, rowNum);
        }
    ...
    public void addDataFormulaValue066(ReportFormulaUtil reportFormulaUtil, String keyStr, int rowNum) {
        reportFormulaUtil.addFormulaValue(keyStr + "_E", "E" + rowNum);
        reportFormulaUtil.addFormulaValue(keyStr + "_G", "G" + rowNum);
        reportFormulaUtil.addFormulaValue(keyStr + "_I", "I" + rowNum);
        reportFormulaUtil.addFormulaValue(keyStr + "_M", "M" + rowNum);
    ...
    public void addDataFormulaCell066(ReportFormulaUtil reportFormulaUtil, String keyStr, int rowNum) {
        reportFormulaUtil.addFormulaCell(keyStr + "_E", rowNum - 1, 4);
        reportFormulaUtil.addFormulaCell(keyStr + "_G", rowNum - 1, 6);
        reportFormulaUtil.addFormulaCell(keyStr + "_I", rowNum - 1, 8);
        reportFormulaUtil.addFormulaCell(keyStr + "_M", rowNum - 1, 12);
    ...
    public void dataList(Map model, XSSFWorkbook wb, List<Object> categories) throws Exception {
        ...
        if("061".equals(reportDetlCd)){
        	title = "기간제근로자";
        }else if("062".equals(reportDetlCd)){
        	title = "포상금";
        }else if("063".equals(reportDetlCd)){
        	title = "사회복무요원보상";
        }else if("064".equals(reportDetlCd)){
        	title = "위원회수당";
        }else if("065".equals(reportDetlCd)){
        	title = "워크숍경비";
        }else if("066".equals(reportDetlCd)){
        	title = "시간제임기제";
        }else{
        	title = "기간제근로자";
        }
