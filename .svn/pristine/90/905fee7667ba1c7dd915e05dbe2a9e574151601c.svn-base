...
@Controller
public class DialogDgrcompoModifyReportController {
    /**
     * Logger for this class
     */
    private static final Logger logger = Logger.getLogger(DialogDgrcompoModifyReportController.class);
    ...
    @Resource(name = "dialogDgrcompoModifyReportService")
    private DialogDgrcompoModifyReportService dialogDgrcompoModifyReportService;

    @RequestMapping("/dialog/ajaxDialogDgrcompoModifyReportData.do")
    public ModelAndView ajaxDgrDeptSeltList(ModelMap model, HttpServletRequest request) throws Exception {
        if (logger.isDebugEnabled()) {
            logger.debug("ajaxDialogDgrcompoModifyReportData(ModelMap, HttpServletRequest) - start");
        }
        ...
        try {
            ...
            JSONObject reportData = JSONObject.fromObject(dialogDgrcompoModifyReportService.selectReportData(jsonParam));

            jsonObject.put("reportData", reportData);

            jsonObject.put(BcjisCommUtil.BCJIS_RETURN_CODE, BcjisCommUtil.BCJIS_RETURN_CODE_SUCC);
        } catch (Exception e) {
            logger.error("ajaxDialogDgrcompoModifyReportData(ModelMap, HttpServletRequest)", e);
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("ajaxDialogDgrcompoModifyReportData(ModelMap, HttpServletRequest) - end");
        }
        return ajaxModel;
    }
    
    @RequestMapping("/dialog/ajaxDialogDgrcompoModifyReportSaveData.do")
    public ModelAndView ajaxDialogDgrcompoModifySaveDgrcompos(ModelMap model, HttpServletRequest request) throws Exception {
        if (logger.isDebugEnabled()) {
            logger.debug("ajaxDialogDgrcompoModifyReportSaveData(ModelMap, HttpServletRequest) - start");
        }
        ...
        try {
            ...
            dialogDgrcompoModifyReportService.updateReportData(jsonParam);
            
            jsonObject.put(BcjisCommUtil.BCJIS_RETURN_CODE, BcjisCommUtil.BCJIS_RETURN_CODE_SUCC);
            jsonObject.put(BcjisCommUtil.BCJIS_MESSAGE, bcjisMessageSource.getMessage("success.common.save"));
        } catch (Exception e) {
            logger.error("ajaxDialogDgrcompoModifyReportSaveData(ModelMap, HttpServletRequest)", e);
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("ajaxDialogDgrcompoModifyReportSaveData(ModelMap, HttpServletRequest) - end");
        }
        return ajaxModel;
    }
