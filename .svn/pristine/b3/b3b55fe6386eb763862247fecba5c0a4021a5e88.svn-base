...
@Component("report070SaveFile")
public class Report070SaveFile {
    ...
    public void dataTot(Map model, XSSFWorkbook wb, List<Object> categories) throws Exception {
        ...
        sheet = wb.createSheet(ReportSaveUtil.getStringValue(reportInfo.get("sheetNm")) + "(�Ѱ�)");
        ...
        cell = row.createCell(0);
        cell.setCellStyle(styles.get("tot0Col0"));
        cell.setCellValue("��    ��");

        cell = row.createCell(1);
        cell.setCellStyle(styles.get("tot0Col1"));
        sheet.addMergedRegion(CellRangeAddress.valueOf("$A$" + (rowNum) + ":$B$" + (rowNum) + ""));

        cell = row.createCell(2);
        cell.setCellStyle(styles.get("tot0Col2"));

        cell = row.createCell(3);
        cell.setCellStyle(styles.get("tot0Col3"));

        sheet.addMergedRegion(CellRangeAddress.valueOf("$C$" + (rowNum) + ":$D$" + (rowNum) + ""));

        cell = row.createCell(4);
        cell.setCellStyle(styles.get("tot0Col4"));

        cell = row.createCell(5);
        cell.setCellStyle(styles.get("tot0Col5"));

        cell = row.createCell(6);
        cell.setCellStyle(styles.get("tot0Col6"));
        cell.setCellFormula("F" + rowNum + "-" + "E" + rowNum);

        cell = row.createCell(7);
        cell.setCellStyle(styles.get("tot0Col7"));

        addDataFormulaCell(reportFormulaUtil, "totalSum_0", rowNum);
        ...
        for (int i = 0; i < categories.size(); i++) {
            ...
            if (dgrLevel > 0) {
                dataCnt++;
                rowNum = writeData(sheet, rowNum, category, styles, dataCnt, reportFormulaUtil);
            }
    ...
    public void bizList(Map model, XSSFWorkbook wb, List<Object> categories) throws Exception {
        ...
        while (!categories.isEmpty()) {
            ...
            rowNum = writeBizlistData(sheet, rowNum, category, styles, totDataCnt, dataCnt, subCnt, reportFormulaUtil);
        }

        if (sheet != null) {
            ReportSaveUtil.writeLastSheet(reportCommDAO, param, wb, sheet, rowNum, styles, reportInfo, 14, 7);
            reportFormulaUtil.writeCellFormula();
            sheet = null;
        }
    }
    ...
    public void data(Map model, XSSFWorkbook wb, List<Object> categories) throws Exception {
        ...
        while (!categories.isEmpty()) {
            ...
            if (dgrLevel == 0) {
                ...
                sheetNm = ReportSaveUtil.getStringValue(reportInfo.get("sheetNm"));
                orderYmd = getOrderYmd(ReportSaveUtil.getStringValue(category.get("orderYmd")));

                sheet = wb.createSheet(sheetNm + "(" + orderYmd + ")");
                reportFormulaUtil = new ReportFormulaUtil(sheet);
                dataCnt = 0;

                rowNum = writeHeader(model, sheet, rowNum, styles, reportInfo, ReportSaveUtil.getStringValue(reportInfo.get("reportNm")), "(" + orderYmd + " ���û���)", 7);
    ...
    public int writeBizlistData(XSSFSheet sheet, int rowNum, JSONObject category, Map<String, CellStyle> styles, int totDataCnt, int dataCnt, int subCnt, ReportFormulaUtil reportFormulaUtil) {
        ...
        if (dgrLevel == 1) {
            cell.setCellValue(getFullOrderYmd(ReportSaveUtil.getStringValue(category.get("dgrcompoNm"))));
        } else {
        ...
        if (formulaFlag == false) {
            cell.setCellValue(ReportSaveUtil.getAmtValue(category.get("considerAmt0")));
        }
        ...
        cell.setCellFormula("E" + rowNum + "-" + "D" + rowNum);
    ...
    @SuppressWarnings("rawtypes")
    public int writeHeader(Map model, XSSFSheet sheet, int rowNum, Map<String, CellStyle> styles, Map reportInfo, String title, String title1, int unitPos) throws Exception {
        Row row = null;
        Cell cell = null;
        ...
        titleCell.setCellStyle(styles.get("title0"));
    ...
    public int writeData(XSSFSheet sheet, int rowNum, JSONObject category, Map<String, CellStyle> styles, int dataCnt, ReportFormulaUtil reportFormulaUtil) throws Exception {
        ...
        addDataFormulaValue(reportFormulaUtil, "totalSum_0", rowNum);
        ...
        List<Report070SaveFrscAmtsVO> frscAmts = getFrscAmts(category);

        String frscNm = "";
        long dmnFrscAmt = 0l;
        long considerAmt = 0l;
        long frscAmt = 0l;
        Report070SaveFrscAmtsVO report070SaveFrscAmtsVO = null;
        if (!frscAmts.isEmpty()) {
            report070SaveFrscAmtsVO = frscAmts.remove(0);
            frscNm = report070SaveFrscAmtsVO.getFrscNm();
            dmnFrscAmt = report070SaveFrscAmtsVO.getDmnFrscAmt();
            considerAmt = report070SaveFrscAmtsVO.getConsiderAmt();
            frscAmt = report070SaveFrscAmtsVO.getFrscAmt();
        }
    ...
    public String getOrderYmd(String orderYmd) {
        return Integer.parseInt(orderYmd.substring(4, 6)) + "." + Integer.parseInt(orderYmd.substring(6, 8));
    }

    public String getFullOrderYmd(String orderYmd) {
        return orderYmd.substring(0, 4) + "." + Integer.parseInt(orderYmd.substring(4, 6)) + "." + Integer.parseInt(orderYmd.substring(6, 8));
    }
    ...
    public List<Report070SaveFrscAmtsVO> getFrscAmts(JSONObject category) {
        List<Report070SaveFrscAmtsVO> list = new ArrayList<Report070SaveFrscAmtsVO>();

        long dmnFrscAmt = ReportSaveUtil.getAmtValue(category.get("dmnFrscAmt1"));
        long considerAmt = ReportSaveUtil.getAmtValue(category.get("considerAmt1"));
        long frscAmt = ReportSaveUtil.getAmtValue(category.get("frscAmt1"));

        if (dmnFrscAmt != 0 || considerAmt != 0 || frscAmt != 0) {
            list.add(new Report070SaveFrscAmtsVO("��", dmnFrscAmt, considerAmt, frscAmt));
        }

        dmnFrscAmt = ReportSaveUtil.getAmtValue(category.get("dmnFrscAmt2"));
        considerAmt = ReportSaveUtil.getAmtValue(category.get("considerAmt2"));
        ...
        considerAmt = ReportSaveUtil.getAmtValue(category.get("considerAmt3"));
        ...
        considerAmt = ReportSaveUtil.getAmtValue(category.get("considerAmt4"));
        ...
        considerAmt = ReportSaveUtil.getAmtValue(category.get("considerAmt5"));
        frscAmt = ReportSaveUtil.getAmtValue(category.get("frscAmt5"));

        if (dmnFrscAmt != 0 || considerAmt != 0 || frscAmt != 0) {
            list.add(new Report070SaveFrscAmtsVO("ä", dmnFrscAmt, considerAmt, frscAmt));
        }

        dmnFrscAmt = ReportSaveUtil.getAmtValue(category.get("dmnFrscAmt6"));
        considerAmt = ReportSaveUtil.getAmtValue(category.get("considerAmt6"));
