...
@Controller
public class BudgetSelectNewController {
    /**
     * Logger for this class
     */
    private static final Logger logger = Logger.getLogger(BudgetSelectNewController.class);
    
    @Resource(name = "budgetSelectNewService")
    private BudgetSelectNewService budgetSelectNewService;
    ...
    @Resource(name = "budgetSelectNewSaveFile")
    private BudgetSelectNewSaveFile budgetSelectNewSaveFile;

    @RequestMapping("/budget/budgetSelectNew.do")
    ...
    @RequestMapping("/budget/ajaxBudgetSelectNewDgrCompoList.do")
    public ModelAndView ajaxBudgetSelectDgrCompoList(ModelMap model, HttpServletRequest request) throws Exception {
        ...
        try {
            ...
            JSONArray resultList = JSONArray.fromObject(budgetSelectNewService.selectDgrCompoList(jsonParam));
    ...
    @RequestMapping("/budget/ajaxBudgetSelectNewSaveReport.do")
    public ModelAndView ajaxBudgetSelectSaveReport(ModelMap model, HttpServletRequest request) throws Exception {
        if (logger.isDebugEnabled()) {
            logger.debug("ajaxBudgetSelectNewSaveReport(ModelMap, HttpServletRequest) - start");
        }
        ...
        try {
            ...
            budgetSelectNewService.saveReport(jsonParam);

            jsonObject.put(BcjisCommUtil.BCJIS_RETURN_CODE, BcjisCommUtil.BCJIS_RETURN_CODE_SUCC);
            jsonObject.put(BcjisCommUtil.BCJIS_MESSAGE, bcjisMessageSource.getMessage("success.common.save"));
        } catch (Exception e) {
            logger.error("ajaxBudgetSelectNewSaveReport(ModelMap, HttpServletRequest)", e);
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("ajaxBudgetSelectNewSaveReport(ModelMap, HttpServletRequest) - end");
        }
        return ajaxModel;
    }
    
    @RequestMapping("/budget/ajaxBudgetSelectNewSaveFile.do")
    public ModelAndView ajaxBudgetSelectNewSaveFile(ModelMap model, HttpServletRequest request) throws Exception {
        if (logger.isDebugEnabled()) {
            logger.debug("ajaxBudgetSelectNewSaveFile(ModelMap, HttpServletRequest) - start");
        }
        ...
        try {
            ...
            budgetSelectNewSaveFile.buildExcelDocument(jsonParam, "RP", "");
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("ajaxBudgetSelectNewSaveFile(ModelMap, HttpServletRequest) - end");
        }
        return ajaxModel;
    }
}
